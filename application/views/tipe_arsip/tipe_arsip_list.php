                ...
                <form class="form-horizontal" id="form-simpan">
                    ...
                    <div class="row clearfix padding-form">
                        ...
                        <div class="col-lg-10 col-md-10 col-sm-8 col-xs-7">
                            <div class="form-group">
                                <div class="form-line">
                                    <input type="text" id="nama" name="nama" class="form-control" placeholder="Masukkan nama tipe arsip">
                                </div>
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        oTable = $('#listTable').DataTable({
            ...
            ajax: "<?php echo site_url('TipeArsip'); ?>",
            lengthMenu: [10, 20, 30],
            dom: '<"top">lrt<"bottom"p>',
            columnDefs: [{"className": "dt-tengah", "targets": [2]}],
            columns: [
                {data: 'kategori_id',name:'kategori_id', searchable: false, orderable: false,
...
//        });
    ...
    function hapus() {
        ...
        $.ajax({
            url: "<?php echo site_url('TipeArsip/hapus'); ?>",
            data: {'id': id},
            dataType: 'JSON',
            beforeSend: function () {
                $(".fa-spinner").show();
        ...
        if ($("form#form-simpan").validate().form() === true) 
	{
            ...
            var id = $("#kode").val();
            $.ajax({
                    url: "<?php echo site_url("TipeArsip/simpan"); ?>/" + id,
                    data: data,
                    type: "POST",
                    dataType: 'JSON',
                    beforeSend: function () {
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.simpan) {
                    ...
                    $("#kode").val(data.model.kategori_id);
                    $(".fa-spinner").hide();
                    $("#btn-simpan").removeAttr("disabled");
                } else {
